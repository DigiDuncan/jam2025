...
class ViewSelectView(ArcadeView):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._batch = Batch()
        self._text = [Text(name.replace("_", " ").upper(), self.center_x, self.height - 80 * (idx + 1),
                           font_size=44, anchor_x='center', anchor_y='center', batch=self._batch,
                           font_name = "GohuFont 11 Nerd Font Mono")
                           for idx, name in enumerate(navigation.available_views) if name != "v_select"]
